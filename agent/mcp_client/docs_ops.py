...
def resolve_document(session: MCPSession, product_display_name: str, product_key: str, db_path: Path) -> str:
...
def append_pulse_section(session: MCPSession, doc_id: str, summary: PulseSummary, product_display_name: str) -> dict:
    ...
    if anchor in doc_text:
        log.info("docs.idempotency_skip", anchor=anchor, reason="Anchor already present in document body")
        return {"status": "skipped", "reason": "idempotency", "anchor": anchor, "deep_link": f"https://docs.google.com/document/d/{doc_id}/edit"}
    ...
    result = session.call_tool("docs.batch_update", {
        "doc_id": doc_id,
